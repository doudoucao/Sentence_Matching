...
import argparse
from models.MultiAtt import MultiAtt
import numpy as np
import pandas as pd
...
class Instructor:
    def __init__(self, opt):
        ...
        self.model = MultiAtt(opt, dataset.embedding_matrix, dataset.char_embedding_matrix).to(self.opt.device)
    ...
    def _test(self, epoch, batch='last'):
        ...
        submission.to_csv('results/MultiAtt_epoch'+str(epoch)+'batch'+str(batch)+'.csv', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--learning_rate', default=0.0004, type=float)  # the smaller, the better
    ...
    parser.add_argument('--attention_size', default=100, type=int)
