...
import argparse
from models.Double_BiMPM import BIMPM
import numpy as np
import pandas as pd
...
class Instructor:
    def __init__(self, opt):
        ...
        self.model = BIMPM(opt, dataset.embedding_matrix, dataset.char_embedding_matrix).to(self.opt.device)
    ...
    def _train(self, criterion, optimizer):
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_data_loader):
                ...
                _,_,_,val_f1 = self._evaluate_acc()
                if float(val_f1) > 0.83:
                    self._test(epoch, i_batch)
                    print('----epoch: ' + str(epoch)+ '----batch: '+ str(i_batch) + '---val_f1: ' + str(val_f1))
    ...
    def _test(self, epoch, batch='last'):
        ...
        submission.to_csv('results/bimpm_epoch'+str(epoch)+'batch'+str(batch)+'.csv', index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_char_len', default=18, type=int)
